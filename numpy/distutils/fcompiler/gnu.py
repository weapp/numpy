...
class GnuFCompiler(FCompiler):
    ...
    def gnu_version_match(self, version_string):
        ...
        if len(vs) <= 20:
            ...
            m = re.search(r'GNU Fortran\s+95.*?([0-9-.]+)', vs)
            if m:
                return ('gfortran', m.group(1))
            m = re.search(
                r'GNU Fortran.*?\-?([0-9-.]+\.[0-9-.]+)', vs)
            if m:
                v = m.group(1)
